...
def f_dyn(R0, A0, v0, w0, q, qd, F0, T0, Fe, Te, tau):
    ...
    if num_q == 0:
        ...
        for i in range(num_q):
            if SE[i] == 1:
                ...
                num_e = num_e + 1
                A_I_i = AA[i, 0:3, 0:3]
                Re0i = RR[i, :] - R0 + np.dot(A_I_i, ce[i, :])
                Me_i[0:3, 0:3] = E_3
                Me_i[0:3, 3:6] = O_3
                Me_i[3:6, 0:3] = skew_sym(Re0i)
                Me_i[3:6, 3:6] = E_3
                Me_i[6:num_q+6, 0:3] = JJ_tx_i.T
                Me_i[6:num_q+6, 3:6] = JJ_rx_i.T
                Fe_Te[i, 0:3] = Fe[i, 0:3]
                Fe_Te[i, 3:6] = Te[i, 0:3]
                F_ex[i, :] = np.dot(Me_i, Fe_Te[i, 0:6])
        for i in range(num_q):
            Fx = Fx + F_ex[i, 0:3]
            Tx = Tx + F_ex[i, 3:6]
            taux = taux + F_ex[i, 6:6 + num_q]
    ...
    Acc = np.dot(np.linalg.pinv(HH), a_Force)
    vd0 = Acc[0:3]
    wd0 = Acc[3:6]
    qdd = Acc[6:6 + num_q]
